...
class _OnboardingFlowState extends State<OnboardingFlow> {
    ...
    return Column(
      ...
      children: [
        ...
        if (_cicloMenstrualRegular) ...[
          ...
          Text(
            'Do 1º dia de uma menstruação até o 1º dia da próxima. Ajuda a '
            'estimativa a ficar mais certa pra você.',
            style: Theme.of(context).textTheme.bodySmall,
          ),
          const SizedBox(height: 8),
          Builder(
            builder: (context) {
              ...
              return Wrap(
                spacing: 8,
                children: [
                  chip('~24 dias', selecionado: _duracaoCicloDias == 24, dias: 24, naoSei: false),
                  chip('~28 dias', selecionado: _duracaoCicloDias == 28, dias: 28, naoSei: false),
                  chip('~32 dias', selecionado: _duracaoCicloDias == 32, dias: 32, naoSei: false),
                  chip('Não sei', selecionado: _duracaoCicloNaoSei, dias: null, naoSei: true),
                ],
              );
            },
          ),
          if (_dataUltimaMenstruacao != null) ...[
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    'Pelo que você informou, hoje você está na fase '
                  ...
                  Text(
                    'Não parece certo? Ajuste a data acima para o primeiro dia '
                    'da sua última menstruação.',
                    style: Theme.of(context).textTheme.bodySmall,
                  ),
                ],
              ),
  ...
  Widget _passoLesoesLimitacoes() {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Lesões ou limitações físicas', style: texto.headlineSmall),
        const SizedBox(height: 16),
        Wrap(
          spacing: 8,
          runSpacing: 8,
        ...
        const SizedBox(height: 4),
        Text(
          'Marque se tem lesão, recomendação médica, ou só não quer treinar '
          'aquele grupo agora. Seu treino é montado sem eles — e você pode '
          'mudar depois em Perfil. Ombro ou cotovelo machucado? Vale marcar '
          'também Peito e Costas, que puxam essas articulações.',
        ...
        if (_gruposEvitar.length == GrupoMuscular.values.length) ...[
          const SizedBox(height: 8),
          Text(
            'Você marcou todos os grupos: seu treino vai ficar só com cardio. '
            'Se não for a intenção, desmarque algum.',
