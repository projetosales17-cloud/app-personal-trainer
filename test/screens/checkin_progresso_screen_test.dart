...
void main() {
  ...
  testWidgets('o botão de enviar só habilita depois de responder as 3 perguntas', (tester) async {
    ...
    await _tocar(tester, find.text('Consegui treinar quase tudo'));
    await _tocar(tester, find.text('Na medida'));
    await _tocar(tester, find.text('Bem recuperada'));
  ...
  testWidgets('enviar grava o check-in, avança o bloco e mostra o resultado', (tester) async {
    ...
    await _tocar(tester, find.text('Treinei pouco'));
    await _tocar(tester, find.text('Difícil demais'));
    await _tocar(tester, find.text('Muito cansada'));
