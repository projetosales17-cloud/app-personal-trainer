...
void main() {
  ...
  testWidgets('Com anamnese salva, mostra os dados da usuária', (tester) async {
    ...
    expect(find.text('Emagrecer e perder medidas'), findsOneWidget);
    expect(find.text('Menopausa'), findsOneWidget);
    expect(find.text('Lactose'), findsOneWidget);
  });

  testWidgets('Botão "Editar meus dados" abre o fluxo pré-preenchido', (tester) async {
    final anamneseRepositorio = AnamneseRepository();
    await anamneseRepositorio.salvar(
      const Anamnese(
        idade: 42,
    ...
    expect(find.text('Editar meus dados'), findsOneWidget);
  ...
  testWidgets('Mostra avisos de assinatura e suporte ainda pendentes', (tester) async {
    ...
    expect(
      find.textContaining('cobrança de assinatura ainda não', skipOffstage: false),
      findsOneWidget,
    );
  ...
  testWidgets('Sem foto de perfil, o topo mostra as iniciais e "Adicionar foto"', (tester) async {
    final anamneseRepositorio = AnamneseRepository();
    await anamneseRepositorio.salvar(
      const Anamnese(
        nome: 'Bianca Reis',
  ...
  testWidgets('Escolher uma foto da galeria salva e mostra o avatar com a imagem', (tester) async {
    final anamneseRepositorio = AnamneseRepository();
    await anamneseRepositorio.salvar(
      const Anamnese(
        nome: 'Bianca',
