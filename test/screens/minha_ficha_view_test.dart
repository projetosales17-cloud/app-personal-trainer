...
void main() {
  ...
  testWidgets('O "?" ao lado de Datas sugeridas abre a explicação de marcação', (tester) async {
    final repositorio = AnamneseRepository();
    await repositorio.salvar(
      const Anamnese(
        idade: 30,
    ...
    expect(find.text('Como marcar seus treinos'), findsOneWidget);
    expect(find.textContaining('folha de presença'), findsOneWidget);
  ...
  testWidgets('gruposEvitar mostra o aviso "Treino sem: ..."', (tester) async {
    final anamneseRepositorio = AnamneseRepository();
    await anamneseRepositorio.salvar(
      const Anamnese(
        idade: 30,
    ...
    expect(find.textContaining('Treino sem: Ombro'), findsOneWidget);
  });

  testWidgets('"Não fazer este exercício" tira da ficha e "Restaurar" traz de volta', (
    tester,
  ) async {
    final anamneseRepositorio = AnamneseRepository();
    await anamneseRepositorio.salvar(
    ...
    await _tocar(tester, find.byIcon(Icons.swap_horiz).first);
    await _tocar(tester, find.text('Não fazer este exercício'));

    expect(await trocasRepositorio.removidos(), isNotEmpty);
    expect(find.text('Fora do treino'), findsWidgets);
